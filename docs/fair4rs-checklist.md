...
| FORRT Outcome's `Repository URL` field | Points to `https://github.com/annefou/european-coastal-biodiversity-replication`. |
...
| GHCR Docker pull | `docker pull ghcr.io/annefou/european-coastal-biodiversity-replication:latest` over HTTPS. |
...
- [ ] Docker image builds and runs the snakemake target via `docker run --rm ghcr.io/annefou/european-coastal-biodiversity-replication:latest`.
