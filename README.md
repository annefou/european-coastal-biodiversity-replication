...
The product choice **changes biodiversity exposure attribution at 73% of resolvable biodiversity-weighted marine sites** across the three storms — present and majoritarian in every regime: **Xaver (North Sea) 83%, Gloria (NW Med) 82%, Xynthia (Bay of Biscay) 60%** — and robust across the threshold sweep. The 73% decomposes as **27% agree / 44% magnitude-disagree / 29% categorically decisive** (one product places the site in water, the other on land). About 16% of sites are too sub-grid for either basin-scale product (Tier 3, out of scope; coverage caveat). Because WAVERYS and the regionals share ERA5 forcing and partial physics, the inter-product delta is conservative and *dominated by the resolution choice* — for a user picking one product, the resolution-and-physics bundle changes the answer at most sites. Outcome: **Validated**; CiTO `qualifies` Loveland et al. 2024 (adequacy of a coarser wave representation is application- and regime-dependent).
